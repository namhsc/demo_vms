...
export function CameraList({ cameras, onRemoveCamera, onToggleCamera, onCameraClick }: CameraListProps) {
  return (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <div className={`w-2 h-2 rounded-full ${
                      camera.hidden ? 'bg-gray-500' : 'bg-green-500 animate-pulse'
                    }`} />
                    <h3 className={`text-sm truncate ${
                      camera.hidden ? 'text-slate-400 line-through' : 'text-white'
                    }`}>
                      {camera.name}
                    </h3>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    <span>{camera.i}</span>
                    <span>•</span>
                    <span>Grid: {camera.w}x{camera.h}</span>
                    {camera.hidden && (
                      <>
                        <span>•</span>
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className={`p-1.5 hover:bg-slate-700 rounded transition-colors ${
                      camera.hidden ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                    }`}
      ...
      <div className="mt-4 pt-4 border-t border-slate-800">
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-slate-800 rounded-lg p-3">
            <div className="text-xs text-slate-400 mb-1">Visible</div>
            <div className="text-xl text-green-400">
              {cameras.filter(c => !c.hidden).length}
            </div>
          </div>
          <div className="bg-slate-800 rounded-lg p-3">
            <div className="text-xs text-slate-400 mb-1">Hidden</div>
